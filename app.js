...
async function fbGetMeters() {
  ...
  snap.forEach(d => {
    ...
    rows.push({
      ...
      sortOrder: Number(m.sortOrder) || 999
    });
  });
...
async function fbTodayReading(meterId, date) {
  const snap = await db.collection('readings')
    .where('meterId', '==', meterId).where('businessDate', '==', date).where('isVoid', '==', false)
    .orderBy('submittedAt', 'desc').limit(1).get();
  if (snap.empty) return null;
  return Object.assign({ readingId: snap.docs[0].id }, snap.docs[0].data());
}
...
async function fbGetDay(p) {
  ...
  const items = await Promise.all(meters.map(async m => {
    const [last, cur] = await Promise.all([fbLastReading(m.meterId), fbTodayReading(m.meterId, date)]);
  ...
  return {
    date: date, businessDate: todayStr(), isAdmin: user.role === 'ADMIN', items: items,
    stats: {
...
async function fbSubmitReadings(p) {
  ...
  for (const en of entries) {
    ...
    if (!m) { results.push({ meterId: mid, ok: false, error: 'ไม่พบมิเตอร์' }); continue; }
    ...
    const cur = await fbTodayReading(mid, date);
    ...
    await db.collection('readings').doc(rid).set({
      ...
      fullReading: full, usageUnits: usage, status: status,
